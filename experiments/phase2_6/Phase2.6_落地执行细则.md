...
17. **SL-03a：exact-match direct bridge（rev2 新增；本阶段最强的直接物理证据）**。对 20 个 exact-match 矩形样本 / **19 个独立工艺条件**，绕过 Ŵ，直接构造 r_W_direct = λ\* / W_line_measured（W_line_measured = 对应单线在 Task 16 的线级 median_W50）。实测重复条件核查（2026-09-04）：唯一重复为 `dataset_index 54（zro2_120_formal, 加工顺序 55）`与 `156（zro2_60_pass, BASE:T12）`，共享 (2000 fs, 10 kHz, N4, 5 mm/s) 但 **h 不同（10 vs 8 µm）**→ 该条件两条 λ\* 取均值入条件级统计并记录 spread 与 h 差异；非 49/50 sentinel。统计单位 = 19 个条件（Spearman/median/IQR/P(|r−1|≤0.25) 均按 n=19）。若对应单线 `width_identifiability ≠ estimable`，该条件标 `W_lower_bound`（r_W_direct 按"≤真实值"语义报告并单独列出，不混入主 median 的 robustness 声明）。**证据优先级冻结：exact-match direct > in-box predicted > out-of-box sensitivity**；H1 终判时 direct 证据权重最高。输出 `scale_bridge/direct_bridge_exact_match.csv` + 核心科学图（λ\* vs W_measured 散点 + r_W_direct 分布）。
...
- `r_W_direct = λ* / W_line_measured`（W_line_measured = 对应单线 estimable 线级 median_W50；对应单线非 estimable → 该条件标 `W_lower_bound` 并单独列出）。
...
- targets 与 metric（rev2）：`p_8_16`（scalar R²）、`ilr_z2`（scalar R²）、**`ilr_z1_z4`（multivariate，Aitchison Q²：预测 4 个 ILR 坐标 → `p25.ilr_inverse` → `p25.aitchison_distance`，Q² = 1 − Σd(p̂,p)²/Σd(p̄,p)²，p̄ = 训练折几何平均成分）**、`A2_8_16`（R²）、`angular_entropy_8_16`（R²）、`lambda_star_4_32`（R²，valid 子集）。**Aitchison Q² 只属于完整 composition（ilr_z1_z4），禁止用于任何 scalar target。**
- 模型（主口径 Ridge，线性基）：
...
- T21 `test_direct_bridge_conditions`：direct bridge 统计单位 = 19 条件；重复条件（54/156）已聚合且记录 h 差异；非 estimable 单线条件标 `W_lower_bound`；r 值有限。
...
- **开放（不阻塞冻结）**：① 人工 QA 标注完成时间（17/18 可先在 estimable 预集上跑，终稿前须全量标注，vs-band 科学图以标注完成为前提）；② 若外审要求 conditional 方向臂，须先人工登记填充轴并回写 §0.8；③ `power_measurement_version=PENDING_REGISTRATION` 的登记（v2 §11 要求），完成前 gate 文档保留弱点注记。
