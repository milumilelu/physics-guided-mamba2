#!/usr/bin/env python3
"""Task 24 (Phase 2.8r1 A): hierarchical information channels ->
...
Expected artifacts (formal):
    outputs/phase2_8_r1/folds/fold_assignments.csv
    outputs/phase2_8_r1/predictability_spectrum_folds.csv
    outputs/phase2_8_r1/predictability_spectrum.csv
    outputs/phase2_8_r1/predictability_spectrum.png
    outputs/phase2_8_r1/realization_diagnostic.csv
    outputs/phase2_8_r1/summary/gsl28_a_evaluation.json
"""
        ...
        for fi, (tr, te) in enumerate(folds):
            for idx in tr:
                records.append({"variant": variant, "fold": fi,
                                "dataset_index": int(df.iloc[idx]["dataset_index"]), "role": "train"})
            for idx in te:
                records.append({"variant": variant, "fold": fi,
                                "dataset_index": int(df.iloc[idx]["dataset_index"]), "role": "test"})
...
def run_cv(df: pd.DataFrame, splits: dict, models: dict, targets: list,
           *, tag: str = "", oof_records: list | None = None) -> pd.DataFrame:
    inner_groups = df["cv_process_group"].to_numpy()
    rows = []
    for name in targets:
        y_all = target_matrix(df, name)
        scorer = scorer_for(name)
        log(f"CV target={name}, variants={list(splits)}")
        for variant, folds in splits.items():
            for model_name, cols in list(models.items()) + [("dummy", None)]:
                X = np.zeros((len(df), 0)) if model_name == "dummy" else df[cols].to_numpy(float)
                for fi, (tr, te) in enumerate(folds):
                    if model_name == "dummy":
                        pred = np.broadcast_to(y_all[tr].mean(axis=0), y_all[te].shape)
                        skill, alpha = q2_outer(y_all[te], pred, y_all[tr]), np.nan
                    else:
                        model, pred, skill, alpha = _fit_eval(X, y_all, scorer, tr, te, inner_groups)
                    scale = np.ones(y_all[tr].reshape(len(tr), -1).shape[1])
                    if scorer == "multi_mse":
                        scale = y_all[tr].std(axis=0, ddof=0)
                        scale = np.where(scale > 0, scale, 1.0)
                    if oof_records is not None:
                        obs2 = y_all[te].reshape(len(te), -1)
                        pred2 = np.asarray(pred).reshape(len(te), -1)
                        null2 = y_all[tr].reshape(len(tr), -1).mean(axis=0)
                        components = (["ilr_z1", "ilr_z2", "ilr_z3", "ilr_z4"]
                                      if name == "Pl" else JOINT_COLS
                                      if name == "Ot_joint" else [SCALARS[name]])
                        for j, idx in enumerate(te):
                            for c, component in enumerate(components):
                                oof_records.append({
                                    "dataset_index": int(df.iloc[idx]["dataset_index"]),
                                    "shared_height_source_id": df.iloc[idx]["shared_height_source_id"],
                                    "cv_process_group": df.iloc[idx]["cv_process_group"],
                                    "target": name, "component": component,
                                    "variant": f"{variant}{tag}", "fold": fi,
                                    "model": model_name, "observed": float(obs2[j, c]),
                                    "predicted": float(pred2[j, c]),
                                    "train_null": float(null2[c]),
                                    "score_scale": float(scale[c]), "alpha": alpha})
                    rows.append({"target": name, "variant": f"{variant}{tag}",
                                 "model": model_name, "fold": fi,
                                 "skill_q2": skill,
                                 "r2_scalar": float(r2_score(y_all[te], pred)) if y_all.ndim == 1 else np.nan,
                                 "alpha": alpha, "n_train": int(len(tr)), "n_test": int(len(te))})
    return pd.DataFrame(rows)


def validate_oof(oof: pd.DataFrame, folds: pd.DataFrame) -> dict:
    keys = ["target", "variant", "model", "component", "dataset_index"]
    prov.require(not oof.duplicated(keys).any(), "duplicate OOF sample/component")
    vals = oof[["observed", "predicted", "train_null", "score_scale"]].to_numpy(float)
    prov.require(np.isfinite(vals).all() and (oof.score_scale > 0).all(), "invalid OOF values")
    max_error = 0.0
    for (t, v, m, f), g in oof.groupby(["target", "variant", "model", "fold"]):
        num = (((g.observed - g.predicted) / g.score_scale) ** 2).sum()
        den = (((g.observed - g.train_null) / g.score_scale) ** 2).sum()
        ref = folds[(folds.target == t) & (folds.variant == v) & (folds.model == m) & (folds.fold == f)]
        prov.require(len(ref) == 1 and g.dataset_index.nunique() == int(ref.iloc[0].n_test), "OOF fold coverage")
        err = abs(float(1 - num / den) - float(ref.iloc[0].skill_q2))
        max_error = max(max_error, err)
        prov.require(err < 1e-10, f"OOF Q2 mismatch {t}/{v}/{m}/{f}: {err}")
    prov.require(oof.groupby(["target", "variant", "model", "fold"]).ngroups == len(folds), "missing OOF folds")
    return {"n_component_rows": len(oof), "unique": True, "max_q2_reconstruction_error": max_error}
...
def _fit_eval(X, y_all, scorer, tr, te, inner_groups):
    ...
    if scorer == "multi_mse":
        ...
        return model, pred * sd + mu, skill, alpha
...
def realization_diagnostic(cfg: dict, frozen: dict, df: pd.DataFrame,
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if ck.iloc[i] == ck.iloc[j]:
                seen.add((i, j))
                pairs.append((i, j, "same_four_inputs_h_may_differ"))
            if (u_all.iloc[i] == u_all.iloc[j]
                    and df.iloc[i].shared_height_source_id != df.iloc[j].shared_height_source_id):
                seen_u.add((i, j))
    ...
    for kind in ("same_four_inputs_h_may_differ", "exact_repeat_u", "random_ordinary"):
    ...
    summary["interpretation"] = "Descriptive only: one exact repeat cannot establish absence of process control or an irreducible noise floor."
...
def main() -> int:
    ...
    prov.require(out.resolve().is_relative_to(allowed), "corrected output must stay in versioned root")
    ...
    oof_records = []
    folds_table = run_cv(df, splits, models, PRIMARY, oof_records=oof_records)
    ...
    if not any(np.isnan(df[SCALARS[t]].to_numpy(float)).any()
        ...
        folds_table = pd.concat(
            [folds_table, run_cv(df, {"src_gkf": splits["src_gkf"]}, models,
                                 band_targets, oof_records=oof_records)], ignore_index=True)
    folds_table = pd.concat(
        [folds_table, run_cv(df, {"src_gkf": splits["src_gkf"]}, models,
                             ["Ot_joint"], oof_records=oof_records)], ignore_index=True)
    ...
    folds_table = pd.concat(
        [folds_table, run_cv(sub, sub_splits, models, PRIMARY, oof_records=oof_records)],
        ignore_index=True)
    folds_table.to_csv(out / "predictability_spectrum_folds.csv", index=False)
    oof = pd.DataFrame(oof_records)
    oof_qa = validate_oof(oof, folds_table)
    oof.to_csv(out / "predictability_oof.csv", index=False)
    sub_records = [{"variant": "src_gkf_inbox", "fold": fi,
                    "dataset_index": int(sub.iloc[idx].dataset_index), "role": role}
                   for fi, (tr, te) in enumerate(sub_splits["src_gkf_inbox"])
                   for role, ix in (("train", tr), ("test", te)) for idx in ix]
    pd.DataFrame(sub_records).to_csv(out / "folds" / "inbox_fold_assignments.csv", index=False)
    ...
    gate = {
        ...
        "protocol": "2.8r1 correction; primary fits retain v2.1 semantics",
        "oof_validation": oof_qa,
        "frozen_inputs": {
            "population": 200,
...
def plot_spectrum(spectrum: pd.DataFrame, path: Path) -> None:
    ...
    prim = prim.set_index("target").loc[PRIMARY].reset_index()
    ...
    plt.close(fig)
    fig, ax = plt.subplots(figsize=(8.5, 4.6))
    for offset, variant in enumerate(["src_gkf", "proc_gkf"]):
        sub = spectrum[spectrum.variant == variant].set_index("target").loc[PRIMARY]
        ax.bar(np.arange(len(PRIMARY)) + (offset - 0.5) * 0.35,
               sub.delta_h_median, width=0.35, label=variant)
    ax.set_xticks(np.arange(len(PRIMARY)), PRIMARY)
    ax.set_ylabel("Median paired-fold Q2(full) - Q2(minus h)")
    ax.set_title("Hatch contribution across morphology observables")
    ax.axhline(0, color="gray", lw=0.8)
    ax.legend()
    fig.tight_layout()
    fig.savefig(path.with_name("hatch_contribution_spectrum.png"), dpi=160)
    plt.close(fig)
