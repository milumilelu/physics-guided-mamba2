...
def simulate_row(params, row, switch, power_W, cfg):
    d, a, info = simulate_sample(
        params, cfg=cfg, tau_s=row.pulse_duration_fs * 1e-15, f_Hz=row.frequency_kHz * 1e3,
...
def job(payload):
    ...
    vector = best.x
    refine_obj = Objective(train[train.dataset_index.isin(refine_ids)], switch, waist_m, cfg)
    ...
    for split_name, frame in (('outer_train', train), ('outer_test', test)):
        for row in frame.itertuples():
            d, a, info = simulate_row(params, row, switch, cfg['power_W'], cell_cfg)
...
def main():
    ...
    try:
        ...
        for key in order:
            members = train_pool[train_pool.component_id == key].dataset_index.tolist()
            if count_r + len(members) <= cfg['calibration']['refine_recipes'] or not picked_r:
                picked_r.extend(members)
                count_r += len(members)
        refine_ids = sorted(picked_r)
        payloads = []
        for fold in range(5):
