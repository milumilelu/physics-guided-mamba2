...
def packet_rollout(positions, energy_J, duration_s, parameters, fixture, packet_size, switch='P11'):
    """Consecutive identical-position pulses grouped into packets of at most packet_size."""
    positions = np.asarray(positions, dtype=float)
    ...
    while k < len(positions):
        same = (np.abs(positions[:, 0] - positions[k, 0]) <= 0.0) & (np.abs(positions[:, 1] - positions[k, 1]) <= 0.0)
