...
def nominal_rollout(logs, power_W, params, fixture, n_x, n_y):
    ...
    return rollout_numpy(pos, np.full(len(pos), d['ep_J']), d['tau_s'], params, fixture)


def choose_fd(fd_values, floor, plateau_rtol):
    """Adjacent-step stability plateau over coarse->fine FD values (pre-registered rule)."""
...
def main():
    ...
    try:
        ...
        for pt in points:
            ...
            grid = rollout_torch(*logs, params, fixture, pt['n_x'], pt['n_y'], switch=c['switch'])
            J = grid.mean()
            J.backward()
