...
def closed_loop_loss(model, control, dt, mask, return_state=False):
    ...
    result = model(control, dt, mask, return_state=return_state)
    if return_state:
        return result['x'].pow(2).sum(), result
    return result.pow(2).sum()
...
def main():
    ...
    try:
        ...
        control = (torch.rand(n_fam, n_tok, 2, generator=gen, dtype=torch.float64) * 2 - 1)
        dt = torch.rand(n_fam, n_tok, generator=gen, dtype=torch.float64) * 0.07 + 0.01
        mask = torch.ones(n_fam, n_tok, dtype=torch.bool)

        point_id = 0
        directions = c['closed_loop']['directions']
        for point in range(c['closed_loop']['points']):
            for direction in directions:
                ...
                if direction == 'control_early_u1_token5':
                    ...
                    weight = dict(base.encoder.named_parameters())['0.weight']
                    ...
                    def loss_at(delta, params=None):
                        ...
                        with torch.no_grad():
                            ...
                            weight[0, 0] = saved
                        return value
                    unit = max(float(weight[0, 0].detach().abs()), 0.1)
        ...
        for frac in c['continuation']['split_fractions']:
            k = int(n_tok * frac)
            with torch.no_grad():
                _, result = closed_loop_loss(model, control, dt, mask, return_state=True)
                full = result['x']
                first = model(control[:, :k], dt[:, :k], mask[:, :k], return_state=True)
                second = model(control[:, k:], dt[:, k:], mask[:, k:],
                               initial_state={'x': first['x'], 'memory': first['memory']})
