...
class ScanLayoutTests(unittest.TestCase):
    ...
    def test_same_position_different_energy_not_merged(self):
        positions=np.zeros((4,2))
        energy=np.array([1e-8,2e-8,0.,5e-8])
        exact=rollout_numpy(positions,energy,1e-12,PARAMS,FIXTURE)
        actual=packet_rollout(positions,energy,1e-12,PARAMS,FIXTURE,packet_size=4)
        for x,y in zip(exact,actual):
