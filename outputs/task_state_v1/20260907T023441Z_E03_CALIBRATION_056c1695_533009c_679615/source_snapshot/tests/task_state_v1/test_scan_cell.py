...
class StrongFeedbackRegressionTests(unittest.TestCase):
    def test_strong_feedback_continuum_falls_back(self):
        p=replace(PARAMS,delta_reference_m=1e-8,kappa=.2,saturation_ratio=.1)
        cfg=replace(CFG,window_over_w=5.,n_phi=16)
        kw=dict(tau_s=1e-12,f_Hz=1e5,v_m_s=1e5*p.waist_m*.1,h_m=4e-6,
                passes=1,power_W=POWER_W,switch='P11')
        D,A,info=simulate_sample_checked(p,cfg=cfg,**kw)
        exact=simulate_sample(p,cfg=replace(cfg,dense_dx_over_w0=0.),**kw)
        self.assertEqual(info['selected_path'],'exact_pulse_fallback')
        self.assertGreater(info['continuum_relative_errors']['D'],.01)
        self.assertEqual((D,A),exact[:2])
