...
def simulate_sample_checked(params, *, cfg, relative_tolerance=.01,
                            absolute_scale_m=1e-12, **recipe):
    """Validate each dense proposal against the pulse lattice; fall back on failure.

    This deliberately incurs reference cost. It is an interior-cell diagnostic,
    NOT finite-ROI registration and not an accelerated calibration solver. A
    reference exception propagates: an unchecked result is never accepted.
    """
    require(np.isfinite([relative_tolerance, absolute_scale_m]).all()
            and relative_tolerance>0 and absolute_scale_m>0, 'Invalid comparison tolerance')
    exact = simulate_sample(params, cfg=replace(cfg, dense_dx_over_w0=0.), **recipe)
    info = dict(exact[2])
    info['comparison_relative_tolerance'] = relative_tolerance
    info['comparison_absolute_scale_m'] = absolute_scale_m
    if recipe['v_m_s']/recipe['f_Hz'] > cfg.dense_dx_over_w0*params.waist_m:
        info['selected_path'] = 'exact_pulse'
        return exact[0], exact[1], info
    try:
        candidate = simulate_sample(params, cfg=cfg, **recipe)
        errors = np.abs(np.asarray(candidate[:2])-exact[:2])/np.maximum(np.abs(exact[:2]),absolute_scale_m)
        info['continuum_relative_errors'] = dict(zip(('D','A'), map(float,errors)))
        if np.isfinite(errors).all() and (errors <= relative_tolerance).all():
            checked = dict(candidate[2])
            checked.update({k:v for k,v in info.items() if k.startswith('comparison_') or k=='continuum_relative_errors'})
            checked['selected_path'] = 'continuum_checked_against_pulses'
            return candidate[0], candidate[1], checked
        info['fallback_reason'] = 'CONTINUUM_LATTICE_DISCREPANCY'
    except (ValueError, FloatingPointError) as exc:
        info['fallback_reason'] = f'{type(exc).__name__}: {exc}'
    info['selected_path'] = 'exact_pulse_fallback'
    return exact[0], exact[1], info
