...
def rollout_recipe(row, params_vec, cfg, waist_m=0.874e-6):
    ...
    depth_um = field * 1e6
    d_med = float(np.median(depth_um))
    a_rms = float(np.sqrt(np.mean((depth_um - d_med) ** 2)))
    ...
    try:
        ...
        for _, row in sample_rows.iterrows():
            ...
            p = PhysicsParameters(0.874e-6, 1.03e-6, 1.2, math.exp(log_F1), math.exp(log_delta),
                                  ...
                                  gamma_F=gamma_F, gamma_delta=gamma_delta)
            recipe = dict(tau_s=row.pulse_duration_fs * 1e-15,
            ...
            h_field, _, _ = simulate_roi(p, cfg=FiniteScanConfig(), **recipe)
            depth_finite = -h_field
            d_finite = float(np.median(depth_finite))
            a_finite = float(np.sqrt(np.mean((depth_finite - d_finite) ** 2)))
            d_roll, a_roll, drift = rollout_recipe(row, vec, cfg)
            scale_d, scale_a = 0.05, 0.005
            rel_d = abs(d_roll - d_finite) / max(abs(d_finite), scale_d)
            rel_a = abs(a_roll - a_finite) / max(abs(a_finite), scale_a)
            rows.append({'dataset_index': int(row.dataset_index),
                         'D_finite_um': d_finite, 'D_rollout_um': d_roll,
                         'A_finite_um': a_finite, 'A_rollout_um': a_roll,
