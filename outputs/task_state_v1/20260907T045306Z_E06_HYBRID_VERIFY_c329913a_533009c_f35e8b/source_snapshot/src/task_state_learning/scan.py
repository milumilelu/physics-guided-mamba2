...
def packet_update(depth, incubation, x_m, y_m, *, pulse_energy_J, duration_s, beam_x_m, beam_y_m,
                  parameters, multiplicity, switch='P11'):
    """Frozen-state packet approximation of m identical pulses (protocol 6.5).

    The per-pulse increment is evaluated ONCE at the packet-start state and scaled by
    the multiplicity; the physical pulse train (count, per-pulse Ep, positions) is
    fixed. multiplicity=1 recovers the exact one-pulse step exactly.
    """
    require(isinstance(multiplicity, int) and multiplicity >= 1, 'Invalid packet multiplicity')
    if multiplicity == 1:
        return ref_physics.step(depth, incubation, x_m, y_m, pulse_energy_J=pulse_energy_J,
                                duration_s=duration_s, beam_x_m=beam_x_m, beam_y_m=beam_y_m,
                                parameters=parameters, switch=switch)
    d = np.array(depth, dtype=np.float64, copy=True)
    q = np.array(incubation, dtype=np.float64, copy=True)
    d1, q1 = ref_physics.step(d, q, x_m, y_m, pulse_energy_J=pulse_energy_J, duration_s=duration_s,
                              beam_x_m=beam_x_m, beam_y_m=beam_y_m, parameters=parameters, switch=switch)
    return d + multiplicity * (d1 - d), q + multiplicity * (q1 - q)


def packet_rollout(positions, energy_J, duration_s, parameters, fixture, packet_size, switch='P11'):
    """Consecutive identical-position AND identical-energy pulses form packets."""
    positions = np.asarray(positions, dtype=float)
    energy_J = np.asarray(energy_J, dtype=float)
    require(len(positions) == len(energy_J), 'Pulse count mismatch')
    require(isinstance(packet_size,int) and packet_size>=1, 'Invalid packet size')
    ...
    while k < len(positions):
        same = ((positions[:,0]==positions[k,0]) & (positions[:,1]==positions[k,1])
                & (energy_J==energy_J[k]))
        require(bool(same[k]), 'Packet rollout requires sorted pulse positions')
        run = int(np.argmax(~same[k:])) if not same[k:].all() else len(positions) - k
        for start in range(k, k + run, packet_size):
            multiplicity = min(packet_size, k + run - start)
            d, q = packet_update(d, q, x, y, pulse_energy_J=float(energy_J[start]),
                                 duration_s=float(duration_s), beam_x_m=float(positions[start, 0]),
                                 beam_y_m=float(positions[start, 1]), parameters=parameters,
                                 multiplicity=multiplicity, switch=switch)
        k += run
    return d, q
