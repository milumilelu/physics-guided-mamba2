...
def main():
    ...
    try:
        ...
        # Pulse-packet refinement (protocol 6.5): the physical pulse train is fixed
        # (count, per-pulse Ep, positions); only the frozen-state packet approximation
        # is refined. Each registered scenario repeats one pulse at the grid center
        # packet_repeat_count times so the approximation is exercised non-trivially
        # (a single-pass scan gives one pulse per site and a vacuous test).
        # multiplicity=1 is the exact per-pulse recurrence.
        packet_rows = []
        for pt in points[:conv['refinement_points']]:
            logs = [pt[f'log_{k}'] for k in DIRECTIONS]
            d = derived(logs, c['power_W'])
            repeat = conv['packet_repeat_count']
            pos = np.zeros((repeat, 2))
            energy = np.full(repeat, d['ep_J'])
            stats = {}
            for m in conv['packet_splits']:
                depth, _ = packet_rollout(pos, energy, d['tau_s'], params, fixture, packet_size=m)
                stats[m] = descriptors(depth)
            for key in stats[1]:
                sizes = sorted(set(conv['packet_splits']))
                require(len(sizes)>=2 and sizes[0]==1, 'Packet refinement needs exact size 1')
                fine, coarse = stats[sizes[0]][key], stats[sizes[1]][key]
                rel = abs(fine - coarse) / abs(coarse) if coarse != 0 else (0.0 if fine == 0 else np.inf)
                packet_rows.append({'point': pt['point'], 'descriptor': key,
                                    **{f'packet_{m}': stats[m][key] for m in conv['packet_splits']},
                                    'rel_change_finest': rel,
                                    'passed': bool(np.isfinite(rel) and rel <= conv['refinement_tolerance'])})
