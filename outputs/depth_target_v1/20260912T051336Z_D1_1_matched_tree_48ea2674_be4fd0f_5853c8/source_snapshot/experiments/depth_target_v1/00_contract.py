...
def main():
    ...
    try:
        audit, seal_report = dt_runs.verify_upstream_seal_tolerant(
            ROOT / config["inputs"]["audit_run"], ROOT / "outputs" / "task_state_v1")
        write_json(out / "upstream_seal_report.json", seal_report)
        gate["checks"]["C1_audit_seal"] = (
            f"PASS exact={seal_report['exact']} crlf_reconstructed={seal_report['crlf_reconstructed']}")
        ...
        pd.DataFrame(provenance).to_csv(out / "input_provenance.csv", index=False, lineterminator="\n")
        frame.to_csv(out / "split_manifest.csv", index=False, lineterminator="\n")
        feature_audit.to_csv(out / "feature_audit.csv", index=False, lineterminator="\n")
