...
def run_experiment(config, frame, inner, features):
        ...
        for family in config["models"]["families"]:
            for model_id in config["models"]["depth_models"]:
                ...
                tuning.append({"outer_fold": fold, "model_id": model_id, "family": family,
                               "selected_alpha": alpha, "selection_metric": "component_balanced_MSE_um2", "inner_risk": inner_risk})
    ...
    for (family, model_id, role), g in oof.groupby(["family", "model_id", "session_role"]):
        ...
        role_rows.append({"model_id": model_id, "family": family, "session_role": role,
                          "MAE_um": float(np.mean(np.abs(err))), "weighting": "row_uniform_within_role",
                          "component_balanced_MAE_um": float(group_weights(g) @ np.abs(err)), "n_rows": len(g),
...
def main():
    ...
    try:
        ...
        metrics.to_csv(out / "depth_metrics.csv", index=False, lineterminator="\n")
        per_role.to_csv(out / "per_role_metrics.csv", index=False, lineterminator="\n")
        tuning.to_csv(out / "inner_tuning.csv", index=False, lineterminator="\n")
        intervals.to_csv(out / "paired_risk_intervals.csv", index=False, lineterminator="\n")
