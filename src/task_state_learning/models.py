...
class B1ClosureModel(nn.Module):
    def __init__(self,memory_id,width=32,d_state=16,memory_seed=0):
        super().__init__();self.memory_id=memory_id;self.width=width
        self.encoder=nn.Sequential(nn.Linear(5,width),nn.SiLU())
        self.memory=make_memory(memory_id,width,d_state=d_state,seed=memory_seed)
    ...
    def forward(self,control,physical_dt,mask,*,initial_state=None,return_state=False,return_closure=False):
        ...
        for k in range(control.shape[1]):
            ...
            closure_sum=closure_sum+(b.pow(2)*active).sum()
            next_x=b1_resolved_step(x,control[:,k],dt,b)
            x=torch.where(active,next_x,x)
            if cache is not None:
                if isinstance(next_cache,dict):
                    cache={key:torch.where(active.view(-1,*([1]*(next_cache[key].ndim-1))),
                                           next_cache[key],value) for key,value in cache.items()}
                else:
                    cache=torch.where(active,next_cache,cache)
        result={'x':x,'memory':cache}
        if return_closure:
            return x,closure_sum/(2*mask.sum().clamp_min(1))
        return result if return_state else x

    def budget(self,dtype=torch.float32):
        if hasattr(self.memory,'mixer'):
            cache=sum(int(v.numel()) for v in self.memory.mixer.initial_cache(1,torch.device('cpu'),dtype).values())
        else:
