...
5. **G27-3 基准值更替（2026-09-05，2.7r2）**：外审终审发现 2.7r1 的 LOHO 评价统计量是 macro mean 而主统计量/bootstrap 用 weighted TV（主 ΔTV 与 bootstrap 不是同一统计量）。2.7r2 修复后（weighted LOHO / DOE-unit bootstrap / own-envelope d_i / profile 提取 FLAGS+净化），本文 §4.3 golden anchors 表中的 G27-3 行由 r1 值（0.4246/0.3547/0.0699/0.0286/0.0020）**更替为 r2 值（0.6151350308641975 / 0.5290432098765432 / 0.08609182098765433 / 0.018792438271604928 / 0.004997501249375313）**，判定仍为 MODEL_INADEQUATE（外审预设接受条件成立）。§3.6 中"IMPROVED_BUT_INADEQUATE——Phase 2.7 period-2 真实状态（ΔTV=0.0699…TV_w=0.355）"的引述同步以 r2 数字为准；2.8B 的 kernel 库已随共享提取路径重建并重算（各级 TV_cond 不变，仅 L2 微移 0.353→0.357）。
